...
def reg_student_info():
    ...
    keys = ["id","name","DoB"]
    with open('students.csv','a') as student_csv:
        writer = csv.DictWriter(student_csv,fieldnames=keys)
        writer.writerow({"id": student_id,"name": student_name,"DoB": student_DoB})

    '''if os.path.exists("students.txt"): #Normal file output
        with open("students.txt", "a") as student:
            student.write(
                "{} | {} | {}".format(student_id, student_name, student_DoB) + "\n"
            )'''
...
def reg_course_info():
    ...
    keys = ["id","name","ects"]
    with open('courses.csv','a') as course_csv:
        writer = csv.DictWriter(course_csv,fieldnames=keys)
        writer.writerow({"id": course_id,"name": course_name,"ects": course_ects})
...
def reg_mark():
    ...
    mark = float(
        input(
            f"Enter marks for {student._get_student_name()} in {course._get_course_name()}: "
        )
    )
...
def CalculateGPA():
    ...
    print("Student's GPA is: ",total_mark/total_ects)
